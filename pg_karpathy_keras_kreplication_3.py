...
def build_network(input_dim, output_dim, hidden_dims=k_hidden_dims, lrate = learning_rate, drate = decay_rate, eps = epsilon):
    ...
    model.add(Dense(hidden_dims[0], kernel_initializer = glorot_uniform(), kernel_regularizer=regularizers.l2(0.01)))
    # model.add(advanced_activations.LeakyReLU())
    model.add(Activation('relu'))
    model.add(Dense(hidden_dims[1], kernel_initializer = glorot_uniform(), kernel_regularizer=regularizers.l2(0.01)))
    # model.add(advanced_activations.LeakyReLU())
    model.add(Activation('relu'))
...
if resume:
    ...
    kmodel.load_weights('save_k_03.h5')
    game_history = pickle.load(open('game_history_k_03_b.p', 'rb'))
...
while True:
  ...
  if done: # an episode finished
    ...
    episode_number += 1
    pickle.dump(game_history, open('game_history_k_03_b.p', 'wb'))
    ...
    if episode_number % 100 == 0: kmodel.save_weights('save_k_03.h5')
    reward_sum = 0
