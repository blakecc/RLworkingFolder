...
model_name =  ("C" + str(len(conv_num)) +  "ConvA" + str(conv_num[0]) + "ConvB" + str(conv_num[1]) +
    "FC" + str(fclayer_dim) + activation_type + "BS" + str(batchSize) + "LR" + str(learning_rate) + "CV" +
...
def customLoss(y_true, y_pred):
    loss = -1 * K.mean(y_true * K.log(y_pred + 1e-08))
    return loss
...
def build_network(input_dim, output_dim, convs, fclayer, lrate, drate, eps, act_type, cvalue, cnorm):
    ...
    model.add(MaxPooling2D(pool_size = (2,2), strides = (2,2)))
    model.add(Flatten())
    model.add(Dense(fclayer, kernel_initializer = glorot_uniform()))
    model.add(Activation(act_type))
