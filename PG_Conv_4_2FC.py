...
model_name =  ("FullImage_C" + str(len(conv_num)) +  "ConvA" + str(conv_num[0]) + "ConvB" + str(conv_num[1]) +
    "DblFC" + str(fclayer_dim[0]) + activation_type + "BS" + str(batchSize) + "LR" + str(learning_rate) + "CV" +
...
def build_network(input_dim, output_dim, convs, fclayer, lrate, drate, eps, act_type, cvalue, cnorm):
    """Create a base network"""

    model = Sequential()
    # model.add(Flatten(input_shape=input_dim))
    # model.add(Dense(hidden_dims[0], kernel_initializer = glorot_uniform(), input_dim=input_dim))
    model.add(Conv2D(conv_num[0], kernel_size = (5, 5), strides = (1, 1), input_shape=input_dim))
    ...
    model.add(Conv2D(conv_num[1], kernel_size = (5, 5), strides = (1, 1)))
    # model.add(advanced_activations.LeakyReLU())
    model.add(Activation(act_type))
    model.add(MaxPooling2D(pool_size = (2,2), strides = (2,2)))
    model.add(Flatten())
    model.add(Dense(fclayer[0], kernel_initializer = glorot_uniform()))
    model.add(Activation(act_type))
    model.add(Dense(fclayer[1], kernel_initializer = glorot_uniform()))
    model.add(Activation(act_type))
...
if resume:
    kmodel = build_network(input_dim = k_input_shape, output_dim = k_output_shape, convs = conv_num, fclayer = fclayer_dim, lrate = learning_rate, drate = decay_rate, eps = epsilon, act_type = activation_type, cvalue = clip_value, cnorm = clip_norm)
    kmodel.load_weights("Models/" + model_name + ".h5")
    # kmodel.load_weights("Models/C2ConvA16ConvB8FC256reluBS512LR2e-05CV0CN16EG0.0001.h5")
    game_history = pickle.load(open("Histories/game_history_" + model_name + ".p", 'rb'))
    # game_history = pickle.load(open("Histories/game_history_C2ConvA16ConvB8FC256reluBS512LR2e-05CV0CN16EG0.0001.p",'rb'))
