...
def build_network(input_dim, output_dim, hidden_dims=k_hidden_dims, lrate = learning_rate, drate = decay_rate, eps = epsilon):
    ...
    model = Sequential()
    model.add(Dense(hidden_dims[0], kernel_initializer = glorot_uniform(), activation='relu', input_dim=input_dim))
    model.add(Dense(hidden_dims[1], kernel_initializer = glorot_uniform(), activation='relu'))
...
if resume:
    ...
    kmodel.load_weights('save_k_02.h5')
...
while True:
  ...
  if done: # an episode finished
    ...
    episode_number += 1
    pickle.dump(game_history, open('game_history_k_02.p', 'wb'))
    ...
    if episode_number % 100 == 0: kmodel.save_weights('save_k_02.h5')
    reward_sum = 0
